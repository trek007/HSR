...
var GRID_STATUS_VALUE_COLUMN_INDEX = 1;
...
//Save Work
function SaveWork() {
    if (validateSaveWork1()) {
        $.ajax(
            {
                type: "post",
                url: 'PostedData',
                data: JSON.stringify({ 'model': vmtList1 }),
...
$(document).ready(function () {
    ...
    // 16-12-2017 DS-198
    // Open prompt for unsaved changes.

    $('ul.nav li[name="linkCMT"] a, ul.nav li[name="linkST"] a,ul.nav li[name="linkPT"] a').click(function (e) {
        if (dirtyGrid) {
            if (confirm("There are unsaved changes. Do you want to continue?")) {
                SaveWork();
                dirtyGrid = false;
    ...
    function setGridHeight() {
        ...
        windowHeight = windowHeight * 0.60;
    ...
    $(document).on("change",
        ...
        function () {
            ...
            if (domainMasterSelection === OPTIONS.Default) { //Select
                ...
                // DS-197 01-03-2018
                // Able to Approve and Submit the record without match status set to --- in VMT
                // Comment the below rbtnApprove line and rbtnRelease button disable method.

                // $(currentColumn).siblings().find("[id=rbtnApprove]").attr('disabled', 'disabled');  // -- DS-197
                    
                //$('.k-grid-content-locked table tr:eq(' + rowIndex + ')').find("[id=rbtnRelease]")

               // $(currentColumn).siblings().find("[id=rbtnRelease]").attr('disabled', 'disabled'); // -- DS-197
    ...
    function validateSubmit() {
        ...
        gridRows.each(function (index,event) {
            ...
            if (checkedAction !== '' && checkedAction !== undefined && checkedAction !== "1") {
                ...
                // For VMT "--"
                // DS-197 01-02-2018
                // Able to Approve and Submit the record without Foundation Standardzied Label set to --- in VMT

                var ddlValue = $('.k-grid-content-locked table tr:eq(' + (index) + ') td:eq(' + GRID_DDL_MASTER_COLUMN_INDEX + ')').find("[name='ddlDomainMasters']").val();
                var releaseBtnProp = $('.k-grid-content-locked table tr:eq(' + index + ')').find("[id=rbtnRelease]").prop('disabled');
                var approveBtnProp = $('.k-grid-content-locked table tr:eq(' + index + ')').find("[id=rbtnApprove]").prop('disabled');

                var releaseChecked = $('.k-grid-content-locked table tr:eq(' + index + ')').find("[id=rbtnRelease]").is(':checked');
                var approveChecked = $('.k-grid-content-locked table tr:eq(' + index + ')').find("[id=rbtnApprove]").is(':checked');

                //if ((ddlValue === "-1001" || ddlValue == null) && (!approveBtnProp && approveChecked))) {

                if ((ddlValue === "-1001" || ddlValue == null) && ((!releaseBtnProp && releaseChecked) || (!approveBtnProp && approveChecked))) {
                    alert("'Foundation Standardized' cannot be '--' or Empty to Release/Approve");
        ...
        if (selectedRows === -1) {//Empty new value being suggested
            ...
            alert("Foundation Standardized label column should not be empty.");
            //return false;
            return false;
        }
        else {
    ...
    function validateSaveWork() {
        ...
        var gridRows = $(".k-grid-content-locked table tr");
        var selectedRows = 0;
        var id = '', checkedAction = '', ddlDomainMasters = '', tbxNew = '', foundationNotes = '', regionalNotes = '', statusId = '', statusValue = '';
        var domainId = '', foundationActionValue = '', regionActionValue = '';
        var rowNo = '';
        ...
        gridRows.each(function (index) {
            ...
            if ($(gridContent).children().eq(GRID_ISDIRTY_COLUMN_INDEX).text() === "1") {
                ...
                foundationActionValue = $(gridContent).children().eq(GRID_FOUNDATIONACTIONVALUE_COLUMN_INDEX).text();
                regionActionValue = $(gridContent).children().eq(GRID_REGIONACTIONVALUE_COLUMN_INDEX).text();
                statusValue = $(gridContent).children().eq(GRID_STATUS_VALUE_COLUMN_INDEX).text();
                ...
                // DS-272  11-01-2018
                // - Enter foundation notes, and click on save works button, release is enabled.
                // - problem foundationActionValue set to one in the database.
                // - Resolved : added StatusValue and if loop

                if (statusValue === 'Approved by Region' && checkedAction !== '3') {
                    foundationActionValue = foundationActionValue;
                    regionActionValue = regionActionValue;
                }
                ...
                if (userRole !== "Regional" && statusValue ==='Approved by Region'){
                    foundationActionValue = foundationActionValue;
                    regionActionValue = regionActionValue;
                }
    ...
    $("[name='linkCMT']").addClass('activeMenu');
...
function onDataBound(e) {
    ...
    gridRows.each(function (index) {
        ...
        $(ddlDomainMasters).append($("<option />").val(-1001).text(" -- "));

        if (userRole !== "Regional") {
            $(ddlDomainMasters).append($("<option />").val(-1002).text("New Value"));
            // DS-275  12-01-2018
            // Notes textbox enable based on the requirement.
        ...
        if (userRole !== "Regional") {
            ...
            if (foundationActionValue === '1') {
                ...
                $(gridLocked).find("[id=rbtnReview]").prop('checked', true);  // Moz browser support properties
                ...
                if ($(this).children().eq(GRID_REGIONACTIONVALUE_COLUMN_INDEX).text() === "3") {
                    ...
                    $(gridLocked).find("[id=rbtnRelease]").prop('checked', true); // Moz browser support properties
                    $(gridLocked).find("[id=rbtnApprove]").removeAttr('checked');
                ...
                $(gridLocked).find("[id=rbtnApprove]").prop('checked', true); // Moz browser support properties
        ...
        var stautsText = $(this).children().eq(GRID_STATUSID_COLUMN_INDEX).text();

        //For Foundation user
        if (stautsText == "1" && $("#hdnUserRole").val() !== "Regional") {//1- Awaiting For FDN
            $(gridLocked).find("[id=rbtnReview]").removeAttr('disabled');
            $(gridLocked).find("[id=rbtnRelease]").removeAttr('disabled');
            $(gridLocked).find("[id=rbtnApprove]").removeAttr('disabled');
            var ddlDomainMasterst = $('.k-grid-content-locked table tr:eq(' + index + ')').children().eq(GRID_DDL_MASTER_COLUMN_INDEX).find("[name='ddlDomainMasters']").val();

            if (ddlDomainMasterst === OPTIONS.New) { //New
                $(gridLocked).find("[id=rbtnRelease]").attr('disabled', 'disabled');
                $(gridLocked).find("[id=rbtnRelease]").prop('disabled', 'disabled'); // Moz browser support properties
            }
        } else {
            $(gridLocked).find("[id=rbtnReview]").attr('disabled', 'disabled');
            $(gridLocked).find("[id=rbtnReview]").prop('disabled', 'disabled'); // Moz browser support properties
        }

        //For Regional user
        if (stautsText == "2" && $("#hdnUserRole").val() === "Regional") {//2- Awaiting For Regional
            $(gridLocked).find("[id=rbtnReview]").removeAttr('disabled');
            //$(gridLocked).find("[id=rbtnRelease]").removeAttr('disabled');
            $(gridLocked).find("[id=rbtnApprove]").removeAttr('disabled');
        }
