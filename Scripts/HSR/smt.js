...
$(document).ready(function () {
    ...
    function setGridHeight() {
        ...
        windowHeight = windowHeight * 0.59;
    ...
    function validateSubmit() {
        //$('.k-grid-content-locked table tr:eq(' + (0) +')').children().eq(2).text()
        var gridRows = $(".k-grid-content-locked table tr");
        var selectedRows = 0;
        var isBaseRecord = '', recordChecked = false, checkedAction = '', stateStudentId = '', actionValue = '';
        ...
        gridRows.each(function (index,event) {
            ...
            if (isBaseRecord === 'true') {
                ...
                if (checkedAction !== '' && checkedAction != undefined && checkedAction !== "1") {
                    // DS-197 12-27-2017
                    // Able to Approve and Submit the record without match status set to --- in SMT
                   
                    var ddlValue = $('.k-grid-content-locked table tr:eq(' + (index + 1) + ') td:eq(' + GRID_ACTION_VALUE_COLUMN_INDEX + ')').find("[name='ddlOptions']").val();
                   ...
                   // if (ddlValue === "1" && (!approveBtnProp && approveChecked)) {

                    if (ddlValue === "1" && ((!releaseBtnProp && releaseChecked) || (!approveBtnProp && approveChecked))) {
                        alert("'Match Status' cannot be '--' to Release/Approve");
    ...
    $("[name='linkST']").addClass('activeMenu');
...
function onDataBound(e) {
    ...
    gridContentRows.each(function (index) {
        ...
        $(this).children().eq(GRID_UPDATED_BY_COLUMN_INDEX).attr('style', 'white-space: nowrap !important; overflow: visible !important;font-family: TheSans B6 SemiBold,TheSans B7 Bold,TheSans B8 ExtraBold !important');
        $(this).children().eq(GRID_STATUS_DESCRIPTION_INDEX).attr('style', 'white-space: nowrap !important; overflow: visible !important;font-family: TheSans B6 SemiBold,TheSans B7 Bold,TheSans B8 ExtraBold !important');
        $(this).children().eq(GRID_SCHOOL_NAME_COLUMN_INDEX).attr('style', 'white-space: nowrap !important; overflow: visible !important;font-family: TheSans B6 SemiBold,TheSans B7 Bold,TheSans B8 ExtraBold !important');
        $('.k-grid-content-locked table tr:eq(' + index + ') td:eq(' + GRID_LAST_NAME_COLUMN_INDEX + ')').attr('style', 'white-space: nowrap !important; overflow: visible !important;font-family: TheSans B6 SemiBold,TheSans B7 Bold,TheSans B8 ExtraBold !important');
        ...
        if (isBaseRecord === 'true') {
            ...
            if (baseRecordStatus === "1") {
                if (!reviewRecordExists) {
                    ...
                    $('.k-grid-content-locked table tr:eq(' + index + ') td:eq(' + GRID_ACTION_COLUMN_INDEX + ')').find("[id=rbtnReview]").removeAttr('disabled');
                } else {
                ...
                $('.k-grid-content-locked table tr:eq(' + index + ') td:eq(' + GRID_ACTION_COLUMN_INDEX + ')').find("[id=rbtnReview]").attr('disabled', 'disabled');

                // DS-199 26-12-2017
                // Regional User should be able to toggle between Approve and Review before they submit any record.

                if (baseRecordStatus === "2" && $("#hdnUserRole").val() === "Regional"){
                    $('.k-grid-content-locked table tr:eq(' + index + ') td:eq(' + GRID_ACTION_COLUMN_INDEX + ')').find("[id=rbtnReview]").removeAttr("disabled");
            ...
            // DS-201  21-12-2017 
            // Cluster not forming.comment the if loop it's working fine 
            //if (SMTModel.length) { //To overcome data issue(Master School data)
            //if (SMTModel != undefined && SMTModel != null) { //To overcome data issue(Master School data)
                $(this).attr('style', 'background-color:' + currentRowColor);
                //$(this).attr('style', '');
                ...
                //$(this).children().eq(GRID_FIRST_NAME_COLUMN_INDEX).attr('style', 'white-space: nowrap !important; overflow: visible !important;');
                //$(this).children().eq(GRID_LAST_NAME_COLUMN_INDEX).attr('style', 'white-space: nowrap !important; overflow: visible !important;');
    ...
    //$(".k-grid-content colgroup col:eq(7)").css("white-space", "nowrap");
